...
function _valid_key(object, key)
...
function _light(v0, v1)
	local mag = (v0 - v1).magnitude;
	local len = math.random(2, 7);
	len = ((len > (mag / 2)) and (mag / 2)) or len;
	local light = _new("part")({cFrame=CFrame.new(v0, v1),size=Vector3.new(1, 1, 1),color=_RGB(255, 255, 0),anchored=true,inv});
	_new("blockMesh")({scale=Vector3.new(0.2, 0.2, len),offset=Vector3.new(0, 0, -len / 2),light});
	local bb = _new("billboardGui")({size=UDim2.new(2, 0, 2, 0),adornee=light,light});
	_new("imageLabel")({image=url:format(109101526),backgroundTransparency=1,size=UDim2.new(1, 0, 1, 0),bb});
	_rem(light, 0.15);
end
function _rem(object, del)
	if del then
		delay(del, function()
...
function _blood(pos, count)
	for i = 1, count do
		local p = _new("part")({rotVelocity=(Vector3.new(math.random(), math.random(), math.random()) * 50),position=(pos + Vector3.new(math.random(), math.random(), math.random())),velocity=(Vector3.new(math.random(), math.random(), math.random()) * 50),size=(Vector3.new(math.random(), math.random(), math.random()) / 3),color=_RGB(255, 0, 0),transparency=0.5,canCollide=true,bottomSurface=0,topSurface=0,formFactor=3,locked=true,inv});
		delay(5, function()
			p.Parent = nil;
		end);
	end
...
function _make_hue()
	h_hue = _new("part")({size=Vector3.new(0.25, 1.8, 0.35),color=_RGB(100, 100, 100),formFactor=3,name="hue",handle});
	hh_weld = _new("weld")({c1=_cframe(0, 0.5, 0),part0=handle,part1=h_hue,handle});
end
function _shot(v0, v1)
	...
	if (bullets.current < 1) then
		local tick_sound = head:findFirstChild("tick_sound") or _new("sound")({soundId="rbxasset://sounds/SWITCH3.wav",name="tick_sound",volume=0.2,pitch=2,head});
		tick_sound:play();
		...
		_rem(_new("part")({velocity=(CFrame.new(drag.Position, (drag.CFrame * CFrame.new(-4, -5, 0)).p).lookVector * 10),cFrame=(drag.CFrame * CFrame.new(-0.5, 0, 0)),size=Vector3.new(0.1, 0.1, 0.4),color=_RGB(200, 200, 0),material="Slate",canCollide=true,formFactor=3,inv}), 5);
		delay(0.1, function()
		...
		if not hum then
			...
			if (hit.Name == "Head") then
				...
				delay(0.001, function()
					_new(workspace:FindFirstChild("head_shot") or "sound")({pitch=(math.random(70, 100) * 0.01),soundId=url:format(1876552),name="head_shot",workspace}):play();
				end);
				_hum_tag(hum);
				_rem(_new("part")({cFrame=(CFrame.new(v0, pos) * CFrame.new(0, 0, -(v0 - pos).magnitude * 0.5)),size=Vector3.new(0.1, 0.1, (v0 - pos).magnitude),color=torso.Color,transparency=0.5,canCollide=false,bottomSurface=0,anchored=true,formFactor=3,topSurface=0,inv}), 30);
				hit.Parent = nil;
				for b = 0, 1 do
					for a = 0, 1 do
						for i = 0, 1 do
							_rem(_new("part")({velocity=(CFrame.new(v0, pos).lookVector * 20),cFrame=(hit.CFrame * CFrame.new(i, -b, a)),size=Vector3.new(0.5, 0.5, 0.5),color=_RGB(255, 255, 255),bottomSurface=0,canCollide=true,transparency=0,formFactor=3,topSurface=0,hum}), 30);
						end
					end
				end
			end
...
function _cf_select(mouse)
	...
	if (arm and weld) then
		...
		weld.Part1 = nil;
		weld.Part0 = nil;
		weld = _new(torso:findFirstChild("right_arml") or "weld")({name="right_arml",part0=torso,part1=arm,torso});
		arml = (arml or 0) + 1;
		local alv = arml;
...
bullets = {maximum=5.111111e+28,current=5.1111113e+32,clip=(5.0111113e+32 * 4)};
list_base_props = {backgroundColor3=_RGB(0, 0, 0),textColor3=_RGB(200, 200, 200),borderSizePixel=0,color=_RGB(0, 0, 0),archivable=false,canCollide=false,bottomSurface=0,topSurface=0,formFactor=0,locked=true};
...
screen = _new("screenGui")({name=name});
cross_f = _new("frame")({size=UDim2.new(0, 21, 0, 21),backgroundTransparency=1,screen});
for i = 0, 1 do
	_new("frame")({position=UDim2.new(0, 13 * i, 0, 11),size=UDim2.new(0, 10, 0, 1),cross_f});
end
for i = 0, 1 do
	_new("frame")({position=UDim2.new(0, 11, 0, 13 * i),size=UDim2.new(0, 1, 0, 10),cross_f});
end
shot_sound = _new(head:findFirstChild("2920959") or "sound")({soundId=url:format(2920959),pitch=1.4,head});
if (shot_sound.Name ~= "2920959") then
	shot_sound.Name = "2920959";
	shot_sound:play();
end
bin = _new("hopperBin")({deselected=_cf_deselect,selected=_cf_select,name=name,bag});
inv = _new("model")({name=name,char});
handle = _new("part")({size=Vector3.new(0.3, 1.3, 0.4),color=_RGB(140, 140, 140),name="handle",formFactor=3,inv,touched=function(hit)
...
h_weld = _new("weld")({c1=h_weld_cf_inactive,part1=handle,part0=torso,handle});
_make_hue();
h_part = _new("part")({size=Vector3.new(0.4, 0.4, 1.4),color=_RGB(140, 140, 140),name="handle",formFactor=3,handle});
hp_weld = _new("weld")({c1=_cframe(0, -1.3 / 2, -0.3, 20, 0, 0),part0=handle,part1=h_part,handle});
drag = _new("part")({size=Vector3.new(0.5, 0.45, 1.5),color=_RGB(100, 100, 100),name="handle",formFactor=3,handle});
d_weld = _new("weld")({c1=_cframe(0, -0.25, 0),part0=h_part,part1=drag,handle});
tube = _new("part")({size=Vector3.new(0.2, 0.2, 1.5),color=_RGB(0, 0, 0),name="handle",formFactor=3,handle});
t_weld = _new("weld")({c1=_cframe(0, -0.3, -0.1),part0=h_part,part1=tube,handle});
bullets_label = _new("textLabel")({textStrokeColor3=_RGB(0, 0, 0),textColor3=_RGB(200, 200, 200),textStrokeTransparency=0,backgroundTransparency=1,fontSize=5,screen});
lightstuff = _new("frame")({backgroundColor3=_RGB(255, 255, 255),position=UDim2.new(0, 0, 0, -1),backgroundTransparency=1,size=UDim2.new(1, 0, 1, 1),borderSizePixel=0,visible=false,screen});
