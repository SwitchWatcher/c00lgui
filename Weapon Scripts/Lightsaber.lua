--MADE BY OneLegend (NOT THE SCRIPT) LOCAL SCRIPT: Go to line 4 and 1351 and put your name where it says "YOUR NAME HERE"
	...
	ca = CFrame.Angles
	Colors = {"Really Red"}
	BladeColor = tostring(BrickColor.Random())
	...
	RageCost = {Berserk=200,RotorBlade=30,Blocking=0.1,Boomerang=30,RageRegening=-0.7,BoulderForce=45,ForceWave=65,ForceWave=32,Teleport=25,DualSpin=4}
	MagnitudeHit = {ForceWave=500}
	Damage = {RotorBlade=8,BoulderForce=100,Boomerang=100,ForceWave=100,DualSpin=5}
	Props = {}
	...
	function Dash(ob, se, mes, of)
		...
		pcall(function()
			coroutine.resume(coroutine.create(function()
				...
				for i = 1, 9999 do
					if (dashin == false) then
						break
					end
					local newpos = (ob.CFrame * CFrame.new(0, off, 0)).p
					local np = Instance.new("Part")
					np.BrickColor = BrickColor.new(TrailColor)
					np.CanCollide = false
					np.BottomSurface = 0
					np.TopSurface = 0
					np.Anchored = true
					np.Transparency = 0.4
					np.formFactor = "Custom"
					np.Parent = ob
					local mag = math.abs((newpos - oldpos).magnitude)
					local mp = nil
					if (mes ~= nil) then
						np.Size = Vector3.new(2, mag, 2)
						mp = Instance.new("FileMesh", np)
						mp.MeshId = mes
					else
						np.Size = Vector3.new(0.4, 0.4, mag)
						mp = Instance.new("BlockMesh", np)
					end
					np.CFrame = CFrame.new(newpos, oldpos)
					np.CFrame = np.CFrame + (np.CFrame.lookVector * (mag / 2))
					if (mes == BlastMesh.MeshId) then
						np.CFrame = np.CFrame * ca(mr(-90), 0, 0)
					else
						np.CFrame = np.CFrame * ca(0, 0, mr(-45))
					end
					oldpos = newpos
					coroutine.resume(coroutine.create(function()
						for i = 1, 0, -0.1 do
							np.Transparency = np.Transparency + 0.03
							mp.Scale = Vector3.new(i, i, 1)
							wait()
						end
						np:Remove()
					end))
	...
	Tor = p(Torm, "Block", 1.98, 1.98, 1, false, false, 0, 0, "basda")
	Tor.Name = "Torso"
	Torw = weld(Tor, Torso, Tor, 0, 0, 0, 0, 0, 0)
	pcall(function()
		Char.Shirt:Clone().Parent = Torm
	end)
	pcall(function()
		Char.Pants:Clone().Parent = Torm
	end)
	...
	Des21 = p(Torm, "Block", 0.6, 0.1, 0.6, false, false, 0, 0.1, "Black")
	Des21m = Instance.new("CylinderMesh", Des21)
	Des21m.Scale = Vector3.new(1, 0.6, 1)
	Des21w = weld(Tor, H1, Des21, 0, -0.55, 0, 0, 0, 0)
	Des22 = p(Torm, "Block", 0.6, 0.1, 0.6, false, false, 0, 0.1, "Black")
	Des22m = Instance.new("CylinderMesh", Des22)
	Des22m.Scale = Vector3.new(1, 0.6, 1)
	Des22w = weld(Tor, H2, Des22, 0, -0.55, 0, 0, 0, 0)
	Blad1 = p(Torm, "Block", 0.3, 4, 0.3, false, false, 1, 0.25, BladeColor)
	M1 = spm("Head", Blad1, 1, 1, 1)
	Blad1w = weld(Tor, H1, Blad1, 0, 1.9, 0, 0, 0, 0)
	Blad2 = p(Torm, "Block", 0.3, 4, 0.3, false, false, 1, 0.25, BladeColor)
	M2 = spm("Head", Blad2, 1, 1, 1)
	Blad2w = weld(Tor, H2, Blad2, 0, 1.9, 0, 0, 0, 0)
	Glow1 = p(Torm, "Block", 0.47, 4.2, 0.47, false, false, 1, 0, BladeColor)
	GM1 = spm("Head", Glow1, 1, 1, 1)
	Glow1w = weld(Tor, H1, Glow1, 0, 1.9, 0, 0, 0, 0)
	Glow2 = p(Torm, "Block", 0.47, 4.2, 0.47, false, false, 1, 0, BladeColor)
	GM2 = spm("Head", Glow2, 1, 1, 1)
	...
	SelectBar.MouseButton1Up:connect(function()
		if (Anim.Move ~= "None") then
			return
		end
		if (Anim.Act == true) then
			return
		end
		if (Anim.Equipped == false) then
			return
		end
		if (SwordType == "Single") then
			Anim.Move = "Changing"
			Anims.ChangeToDual(0, 1, 0.065 * Speed)
			SwordType = "Dual"
			Anim.Move = "None"
		else
			Anim.Move = "Changing"
			Anims.ChangeToSingle(0, 1, 0.065 * Speed)
			SwordType = "Single"
			Anim.Move = "None"
		end
	end)
					...
					coroutine.resume(coroutine.create(function()
						if ((v ~= Char) and (v ~= nil) and (v:findFirstChild("Torso") ~= nil) and (math.abs((v.Torso.Position - Blad2.Position).magnitude) < MagnitudeHit[Anim.Move])) then
							...
							if ((hit ~= nil) and (hit.Parent:findFirstChild("Humanoid") ~= nil) and (ft(RightDebounce, hit.Parent.Name) == nil)) then
								...
								wait(DebounceSpeed)
								local nf = ft(RightDebounce, hit.Parent.Name)
								if (nf ~= nil) then
									table.remove(RightDebounce, nf)
								end
							end
						elseif (v:IsA("BasePart") and (v.Anchored == false) and (math.abs((v.Position - Blad2.Position).magnitude) < MagnitudeHit[Anim.Move])) then
							v:BreakJoints()
							v.Velocity = (cf(Blad2.Position, v.Position).lookVector * 30) + Vector3.new(0, 20, 0)
						end
					end))
				end
			end
		...
		if (Left == true) then
			if ((hit ~= nil) and (hit.Parent:findFirstChild("Humanoid") ~= nil) and (ft(LeftDebounce, hit.Parent.Name) == nil)) then
				coroutine.resume(coroutine.create(function()
					...
					wait(DebounceSpeed)
					local nf = ft(LeftDebounce, hit.Parent.Name)
					if (nf ~= nil) then
						table.remove(LeftDebounce, nf)
					end
				end))
	...
	Anims.Equip = function(i1, i2, is)
		...
		H1w.Part0 = Ra
		H1w.Part1 = H1
		H1w.C0 = cf(0, -1.1, 0) * ca(0, mr(180), 0)
		H1w.C1 = ca(mr(-90), 0, 0)
	...
	Anims.RotorBlade = function(i1, i2, is, RaigCost)
		...
		H1w.C0 = cf(0, -1.1, 0) * ca(0, 0, 0)
		H1w.C1 = cf(0, -0.4, 0) * ca(mr(-90), 0, 0)
		local soundtime = 0
		for i = i1, i2 * 4, is do
			if (soundtime == 10) then
				soundtime = 0
				Sound(Sounds.SaberSlash, 2.5, 0.5)
			else
	...
	Anims.BoulderForce = function(i1, i2, is, RaigCost)
		...
		bould.Name = "Boulder"
		bould.Material = "Concrete"
		local rm = RockMesh:Clone()
		rm.Scale = Vector3.new(3, 3, 4.8)
		rm.Parent = bould
		bould.Elasticity = 0
		bould.Friction = 2
		bould.CFrame = cf(Torso.Position.x + (math.random(-14, 14)), Torso.Position.y - 5, Torso.Position.z + (math.random(-14, 14))) * CFrame.Angles(math.random(-33, 33) / 10, math.random(-33, 33) / 10, math.random(-33, 33) / 10)
		local warpdes = true
		local bpos = bould.Position
		Sound(Sounds.Cast, 0.95, 0.8)
		coroutine.resume(coroutine.create(function()
			repeat
				Functions.BrickWarpDesign(bould, 9)
				wait()
			until warpdes == false
		end))
		for i = 0, 1, 0.08 do
			bould.CFrame = CFrame.new(bpos.x, bpos.y, bpos.z) + Vector3.new(0, 20 * i, 0)
			wait()
		end
		wait(1)
		bould.CFrame = CFrame.new(bpos.x, bpos.y + 20, bpos.z)
		bould.CFrame = cf(bould.Position, mouse.hit.p)
		bould.Anchored = false
		wait()
		bould.Velocity = bould.CFrame.lookVector * (math.random(180, 350))
		bould.Touched:connect(function(hit)
			Functions.BoulderTouch(hit, bould)
		end)
		wait(0.5)
		warpdes = false
	...
	Anims.ForceWave = function(i1, i2, is, RaigCost)
		...
		local wav = p(Torm, "Block", 0.1, 0.1, 0.1, true, false, 0.3, 0, BladeColor)
		wav.Anchored = true
		...
		coroutine.resume(coroutine.create(function()
			...
			for i = i1, i2 * 3, is * Speed do
				H1w.C1 = ca(mr(-50), mr(360 * i), 0)
				wait()
			end
		end))
		Sound(Sounds.Cast, 0.45, 1)
		for i = i1, i2 * 5, is do
			Functions.BrickWarpDesign(Torso, 13)
			wav.CFrame = cff * ca(0, mr(180 * i), 0)
			wait()
		end
		for i = 1, 0.3, -0.14 * Speed do
			wav.Transparency = i
			wait()
		end
		wav:Remove()
	...
	Anims.DualSpin = function(i1, i2, is, RaigCost)
		...
		Anim.Act = true
		for i = i1, i2, is * Speed do
			H1w.C0 = cf(0, -1.1, 0) * ca(0, mr(-180 - (-90 * i)), mr(0 * i))
			H1w.C1 = ca(mr(-90 - (60 * i)), 0, mr(0 * i))
			H2w.C0 = cf(0, -1.1, 0) * ca(0, mr(180 - (270 * i)), mr(0 * i))
			H2w.C1 = ca(mr(-90 + (-30 * i)), 0, 0)
		...
		Right = true
		Left = true
		Dash(Blad1, 2.6 / Speed, nil, 2)
		Dash(Blad2, 2.6 / Speed, nil, 2)
		local x, y, z = Neck.C0:toEulerAnglesXYZ()
		local soundtime = 0
		for i = i1, i2 * 4, is * Speed do
			if (soundtime == 10) then
				soundtime = 0
				Sound(Sounds.SaberSlash, 2.5, 0.5)
			else
			...
			Torw.C1 = ca(0, mr(i * 360), 0)
			Neck.C0 = cf(0, 1, 0) * ca(x, y, z + mr(-360 * i))
			wait()
		end
		Neck.C0 = cf(0, 1, 0) * ca(x, y, z)
		Torw.C1 = ca(0, 0, 0)
		DebounceSpeed = DebounceSpeed + (0.5 * Speed)
		for i = i2, i1, -is * Speed do
			H1w.C0 = cf(0, -1.1, 0) * ca(0, mr(-180 - (-90 * i)), mr(0 * i))
		...
		for i, v in pairs(workspace:GetChildren()) do
			if ((v ~= Char) and (v ~= nil) and (v:findFirstChild("Torso") ~= nil) and (v:findFirstChild("Humanoid") ~= nil)) then
				if ((math.abs((v.Torso.Position - bould.Position).magnitude) < 11) and (ft(RightDebounce, v.Name) == nil)) then
					...
					coroutine.resume(coroutine.create(function()
						wait(DebounceSpeed)
						local nf = ft(RightDebounce, v.Name)
						if (nf ~= nil) then
							table.remove(RightDebounce, nf)
						end
					end))
				end
			elseif ((v ~= bould) and v:IsA("BasePart") and (v.Anchored == false) and (math.abs((v.Position - bould.Position).magnitude) < 11)) then
				v:BreakJoints()
				v.Velocity = (cf(bould.Position, v.Position).lookVector * 10) + Vector3.new(0, 10, 0)
			end
		end
	end
	...
	Functions.RageRegen = function()
		...
		Anim.Act = true
		local wav = p(Torm, "Block", 0.1, 0.1, 0.1, true, false, 1, 0, BladeColor)
		wav.Anchored = true
		local wavm = BlastMesh:Clone()
		wavm.Parent = wav
		local wavv = 0
		...
		coroutine.resume(coroutine.create(function()
			...
			local moved = 2
			repeat
				if ((Rage >= MaxRage) or (Anim.key.z == false)) then
					break
				end
				if (moved == 2) then
					moved = 1
		...
		repeat
			...
			if (math.random(1, 6) == 4) then
				Functions.Sparkle(Torso, 8)
			end
			if (math.random(1, 3) == 3) then
				Functions.BrickWarpDesign(Torso, 10)
			end
			wavv = wavv + 10
			Rage = Rage - (RageCost['RageRegening'] * RageIncome)
			wav.CFrame = cf(Torso.Position.x, hpos + 1.4, Torso.Position.z) * ca(0, mr(wavv), 0)
		until (Rage >= MaxRage) or (Anim.key.z == false)
		for i = 0.3, 1, 0.14 * Speed do
			wav.Transparency = i
			wait()
		end
		wav:Remove()
	...
	Functions.Teleport = function(i1, i2, is, RaigCost)
		...
		local tele = false
		local tele2 = false
		local mouseact = mouse.Button1Up:connect(function()
			tele2 = true
		end)
		coroutine.resume(coroutine.create(function()
		...
		local telehit = nil
		local wav = p(Torm, "Block", 0.1, 0.1, 0.1, true, false, 0.3, 0.1, BladeColor)
		wav.Anchored = true
		local wavm = BlastMesh:Clone()
		wavm.Parent = wav
		local wavv = 0
		...
		repeat
			...
			wavv = wavv + 8
			Functions.Sparkle(La, 3)
			Functions.BrickWarpDesign(La, 4)
			if (telehit ~= nil) then
				wav.Transparency = 0
				wav.CFrame = cf(telepos.x, telepos.y + 1, telepos.z) * ca(0, mr(wavv), 0)
			else
				wav.Transparency = 1
			end
			wait()
		until (tele == true) or (tele2 == true)
		mouseact:disconnect()
		if ((telehit == nil) or (math.abs((Torso.Position - telepos).magnitude) > Props.MaxTeleDistance)) then
			Sound(Sounds.Punch, 1, 1)
			for i = 0.3, 1, 0.14 do
				wavm.Scale = Vector3.new(3.5 + (5 * i), 2, 3.5 + (5 * i))
				wav.Transparency = i
				wait()
			end
			wav:Remove()
			...
			for i = 1, 10 do
				wait()
				Functions.Sparkle(Torso, 5, 3)
				Functions.BrickWarpDesign(Torso, 6)
			end
			ShockWave(Torso, 7, BladeColor)
			Torso.CFrame = wav.CFrame + Vector3.new(0, 2.2, 0)
			Sound(Sounds.EnergyBlast, 1.2, 0.6)
			ShockWave(Torso, 7, BladeColor)
			for i = 1, 10 do
				wait()
				Functions.Sparkle(Torso, 5, 3)
	...
	bin.Selected:connect(function(mm)
		...
		mouse.Button1Down:connect(function()
			...
			if (not Anim.Click and (Anim.Move == "None") and not Anim.Act) then
				...
				coroutine.resume(coroutine.create(function()
					local oldcomb = Anim.CanBerserk
					wait(0.5)
					if ((Anim.ComboBreak == true) and (Anim.CanBerserk == oldcomb)) then
						Anim.ComboBreak = false
						Speed = Speed - 0.5
						Anim.CanBerserk = 0
					end
	...
	bin.Deselected:connect(function(mouse)
		Anim.Equipped = false
		if (SwordType == "Dual") then
			Anims.ChangeToSingle(0, 1, 0.25 * Speed)
			SwordType = "Single"
		end
