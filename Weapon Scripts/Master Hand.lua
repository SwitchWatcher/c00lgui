	...
	coroutine.resume(coroutine.create(function()
		...
		for i = 1, 4 do
			coroutine.resume(coroutine.create(function()
				...
				sw.Anchored = true
				sw.CFrame = pos * cf(ra(-3 * s, 3 * s), 0, ra(-3 * s, 3 * s)) * ca(rd(ra(-27, 27)), rd(ra(-360, 360)), rd(ra(-27, 27)))
...
function ShortifiedAnim(theanim)
	if (theanim == "Beam") then
		...
		cromm.Scale = v3(15 * s, 5 * s, 15 * s) --local cromw  = weld(crom,Palm,crom,0,Palm.Size.y/3.6,-Palm.Size.z*1.8,rd(-90),0,0)
		crom.Anchored = true
		dias = {}
		loldiv = 360 / 8
		...
		coroutine.resume(coroutine.create(function()
			...
			repeat
				...
				for rofl = 1, #dias do
					dias[rofl].CFrame = crom.CFrame * ca(0, rd(rofl * loldiv), 0) * cf(0, -19 * s, 0) * ca(rd(25), 0, 0) * cf(0, 32 * s, 0)
				end
			until not Button 
...
hb.Selected:connect(function(mouse)
	...
	mouse.KeyDown:connect(function(k)
		...
		elseif ((k == "f") and (Anim.a == "Gun")) then --  on anim off
			Anim.a = "None"
			HandSign("five", 0.8)
		...
		elseif ((k == "r") and (Anim.a == "Ride")) then
			...
			Posing = "Follow"
			Facing = "Owner"
			if ((GrabWeld.Part1 ~= nil) and (GrabWeld.Part1.Parent:findFirstChild("Humanoid") ~= nil)) then
	...
	mouse.Button1Down:connect(function()
		...
		elseif (Anim.a == "GroundGrip") then
			...
			wait(0.25)
			Dustplosion(cf(bpos), 30 * s, {"Really red","Really black","Really red"}, 3)
			...
			for yyy = 0, 360, 60 do
				coroutine.resume(coroutine.create(function()
					...
					wait(0.25)
					Earthsplosion(cf(bpos) * ca(0, rd(yyy + lolra), 0) * cf(0, 0, 5 * 18 * s), 25, 1, 20, 120)
				end))
			end
			wait(0.3)
			HandSign("grab2", 0.5)
