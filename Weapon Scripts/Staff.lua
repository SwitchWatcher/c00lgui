...
function fire(v)
	...
	for i = distance / num, distance, distance / num do
		...
		m.Scale = Vector3.new(0.25, 0.25, 1)
...
bin.Selected:connect(function(mouse)
	...
	if (player:FindFirstChild("check") == nil) then
		...
		rns.C1 = CFrame.new(-1.51, 0.1, -0.05) * CFrame.Angles(math.pi * "-0", math.pi * 0, math.pi * -0.05)
		lns = Instance.new("Weld")
		lns.Parent = player.Torso
		lns.Part0 = lns.Parent
		lns.Part1 = player["Left Arm"]
		lns.C1 = CFrame.new(1.51, 0.1, -0.05) * CFrame.Angles(math.pi * "-0", math.pi * 0, math.pi * 0.05)
		...
		rbm.Parent = rblade
		rbm.Scale = Vector3.new(0.4, 1, 0.4)
		rbw = Instance.new("Weld")
		rbw.Parent = rblade
		rbw.Part0 = rarm
		rbw.Part1 = rblade
		rbw.C1 = CFrame.new(0, 0, 1) * CFrame.Angles(math.pi * 0.5, math.pi * 0, math.pi * 0)
		...
		tipm.Scale = Vector3.new(0.41, 0.2, 0.41)
		tipm.Parent = tip
		...
		tip2m.Scale = Vector3.new(0.41, 0.2, 0.41)
		tip2m.Parent = tip2
		...
		tip3m.Scale = Vector3.new(0.41, 0.2, 0.41)
		tip3m.Parent = tip3
		tip3w = Instance.new("Weld")
		tip3w.Parent = rblade
		tip3w.Part0 = rblade
		tip3w.Part1 = tip3
		tip3w.C1 = CFrame.new(0, -0.5, 0)
		...
		tip4m.Scale = Vector3.new(0.41, 0.2, 0.41)
		tip4m.Parent = tip4
		tip4w = Instance.new("Weld")
		tip4w.Parent = rblade
		tip4w.Part0 = rblade
		tip4w.Part1 = tip4
		tip4w.C1 = CFrame.new(0, 0.5, 0)
		...
		tip5m.Scale = Vector3.new(0.41, 0.2, 0.41)
		tip5m.Parent = tip5
		...
		tip6m.Scale = Vector3.new(0.05, 0.6, 0.41)
		tip6m.Parent = tip6
		...
		tip7m.Scale = Vector3.new(0.41, 0.6, 0.05)
		tip7m.Parent = tip7
		...
		tip6am.Scale = Vector3.new(0.15, 0.6, 0.41)
		tip6am.Parent = tip6a
		...
		tip7am.Scale = Vector3.new(0.41, 0.6, 0.15)
		tip7am.Parent = tip7a
		...
		tip6bm.Scale = Vector3.new(0.05, 0.6, 0.41)
		tip6bm.Parent = tip6b
		...
		tip7bm.Scale = Vector3.new(0.41, 0.6, 0.05)
		tip7bm.Parent = tip7b
			...
			if (hit.Parent:findFirstChild("Humanoid") ~= nil) then
				if (hit.Parent.Name ~= player.Name) then
					...
					for i = 1, (math.random(1, 3)) do
						...
						bleedm.Scale = Vector3.new(0.5, 0.5, 0.5)
						bleedm.Parent = bleed
	...
	-- moves
		...
		if (key == "e") then
			right.C1 = CFrame.new(-1.51, 0.1, -0.05) * CFrame.Angles(math.pi * "-0", math.pi * 0, math.pi * -0.05)
			wait()
			...
			right.C1 = CFrame.new(-1.51, 0.1, -0.05) * CFrame.Angles(math.pi * "-0", math.pi * 0, math.pi * -0.05)
		elseif (key == "q") then
			left.C1 = CFrame.new(1.51, 0.1, -0.05) * CFrame.Angles(math.pi * "-0", math.pi * 0, math.pi * 0.05)
			wait()
		...
		if (key == "b") then
			...
			for i = 1, 100 do
				...
				pm.Parent = p
				pm.MeshType = "Sphere"
				pm.Scale = Vector3.new(0.2 + (0.05 * i), 0.2 + (0.05 * i), 0.2 + (0.05 * i))
		...
		if (key == "v") then
			...
			if (tar ~= nil) then
				if ((tip6a.Position - tar.Position).magnitude < 100) then
					...
					shotm.Scale = Vector3.new(0.2, 0.2, (tip6a.Position - mouse.Hit.p).magnitude)
		...
		if (key == "h") then
			...
			local dx = (math.random() - 0.5) * 0.02
			local dy = (math.random() - 0.5) * 0.02
			local dz = (math.random() - 0.5) * 0.02
			parts = {}
			local f = coroutine.create(function()
				fire(Vector3.new(v.x + dx, v.y + dy, v.z + dz))
			end)
			coroutine.resume(f)
			wait(0.5)
			loadstring(down)()
		...
		if (key == "y") then
			...
			for i = 2, 101 do
				...
				game:GetService("Debris"):AddItem(boulder, 0.8)
			end
			loadstring(down)()
		end
		if (key == "n") then
