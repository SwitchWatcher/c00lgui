...
light1.Color = Color3.new(0.2, 0.2, 255)
light1.Range = 10
...
Bin.Name = "Techo Gauntlet"
local Bg = it("BodyGyro")
Bg.maxTorque = Vector3.new(400000, 400000, 400000) * math.huge
Bg.P = 20000
Bg.Parent = nil
		...
		coroutine.resume(coroutine.create(function()
			...
			coroutine.resume(coroutine.create(function(Cone, Screen1, Screen2, Screen3)
				while Mode == "Choose" do
					...
					light1.Brightness = 0.7
					light2.Brightness = 0.7
					light3.Brightness = 0.7
					wait(0.06)
			...
			coroutine.resume(coroutine.create(function()
				for z = 1, 2 do
					coroutine.resume(coroutine.create(function()
						...
						for i = 0, 1, 0.1 do
							wait()
							light.Brightness = light.Brightness - 0.1
...
function slash1()
	...
	Bg.Parent = Head
	Bg.cframe = Head.CFrame
	con1 = sprt16.Touched:connect(function(hit)
		Damagefunc1(hit, 20, 10)
	end)
...
function slash2()
	...
	for i = 0, 1, 0.2 do
		...
		LW.C1 = cf(0, 0.5, 0) * euler(0, 1 + (0.57 * i), 0)
...
function slash3()
	...
	for i = 0, 1, 0.1 do
		...
		RW.C0 = cf(1.5, 0.5, 0) * euler(0, 0, 0.2)
		RW.C1 = cf(0, 0.5, 0) * euler(0, 0, 0)
		LW.C0 = cf(-1.5, 0.5, 0) * euler(1.57 + (0.43 * i), 0, -1.57 + (1 * i))
	...
	for i = 0, 1, 0.2 do
		...
		RW.C1 = cf(0, 0.5, 0) * euler(0, 0, 0)
		LW.C0 = cf(-1.5 + (1.5 * i), 0.5 - (0.2 * i), -1 * i) * euler(2 - (1.5 * i), 0, -0.57 + (1.07 * i))
	...
	for i = 0, 1, 0.1 do
		...
		RW.C1 = cf(0, 0.5, 0) * euler(0, 0, 0)
		LW.C0 = cf(0, 0.3, -1) * euler(0.5 - (0.2 * i), 0, 0.5)
		LW.C1 = cf(0, 0.5, 0) * euler(0, -0.5, 0)
	end
	con1:disconnect()
	wait()
...
function Lightning(p0, p1, tym, ofs, col, th, tra)
	...
	for i = 1, tym do
		...
		light1 = Instance.new("PointLight")
		light1.Brightness = 0.8
		light1.Color = Color3.new(0.2, 255, 0.2)
		light1.Range = 8
		light1.Parent = li
...
function showDamage(Char, Dealt, du)
	...
	if (Mode == "Drones") then
		...
		if (key == "f") then
			...
			elseif (Aiming == true) then
				...
				for i = 0, 1, 0.1 do
					wait()
					RW.C0 = cf(1.5 - (0.5 * i), 0.5, -0.5 * i) * euler(0.5 + (1.07 * i), 0, -1 * i)
					RW.C1 = cf(0, 0.5, 0) * euler(0, 0, 0)
				...
				for i = 0, 1, 0.1 do
					wait()
					RW.C0 = cf(1 + (0.5 * i), 0.5, -0.5 + (0.5 * i)) * euler(1.57 - (1.07 * i), 0, -1 + (1 * i))
					RW.C1 = cf(0, 0.5, 0) * euler(0, 0, 0)
					LW.C0 = cf(-1 - (0.5 * i), 0.5, -0.5 + (0.5 * i)) * euler(1.57 - (1.57 * i), 0, 1 - (1.2 * i))
					LW.C1 = cf(0, 0.5, 0) * euler(0, 0, 0)
				end
				attack = false
			end
